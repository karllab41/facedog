...
# camera = cv2.VideoCapture('/Users/dgrossman/data/movie/061817_FNSPodcast_PODCAST_FNS_VIDEO_1.mp4')
# camera = cv2.VideoCapture('terminator.mp4')
camera = cv2.VideoCapture('fns-podcast.mp4')
...
while True:
    ...
    for face_encoding,face_location in zip(list_face_encodings,list_face_locations):
        ...
        for person in people.keys():
            ...
            facevec = current['average']
            facearray = current['array']
            times = current['times']
            ...
            if match[0]:
                exists = True
                name = person
                facearray.append(face_encoding)
                times.append((frame,face_location))
                #current['average'] = np.mean(facearray, axis=0)

        if not exists:
            name = '{0}'.format(currentUnknown)
            currentUnknown += 1
            current = people[name]
            current['average'] = face_encoding
            tmp = list()
            tmp.append(face_encoding)
            current['array'] = tmp
            current['times'] = list()
            current['times'].append((frame,face_location))
        ...
        for (top, right, bottom, left), name in zip(list_face_locations, list_face_names):
            ...
            #bottom *= 4
            #left *= 4
            cv2.rectangle(small_img, (left, top), (right, bottom), (255, 0, 0), 2)
            cv2.rectangle(small_img, (left, bottom+20), (right, bottom), (255, 0, 0), thickness=-1)
            font = cv2.FONT_HERSHEY_PLAIN
            cv2.putText(small_img, name, (left+6, bottom+10), font, 1.0, (255, 255, 255), 1)

        cv2.imshow('detections', small_img)
